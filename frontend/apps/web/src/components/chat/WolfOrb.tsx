'use client'

import React, { useCallback, useMemo, useRef, useState } from 'react'
import { motion, useMotionValue, useSpring, useTransform, type MotionStyle } from 'framer-motion'

type WolfOrbState = 'idle' | 'online' | 'thinking' | 'listening'
type WolfOrbSize = 'xs' | 'sm' | 'md' | 'lg'

interface WolfOrbProps {
  size?: WolfOrbSize
  state?: WolfOrbState
  trackPointer?: boolean
  showStatus?: boolean
  showRing?: boolean
  className?: string
  ariaLabel?: string
}

const sizeMap: Record<WolfOrbSize, number> = {
  xs: 40,
  sm: 56,
  md: 72,
  lg: 96,
}

const stateAccent: Record<WolfOrbState, { core: string; glow: string; ring: string; status: string }> = {
  idle: {
    core: 'rgba(255, 184, 0, 0.55)',
    glow: 'rgba(255, 184, 0, 0.35)',
    ring: 'rgba(255, 184, 0, 0.35)',
    status: 'bg-primary',
  },
  online: {
    core: 'rgba(67, 237, 158, 0.55)',
    glow: 'rgba(67, 237, 158, 0.4)',
    ring: 'rgba(67, 237, 158, 0.4)',
    status: 'bg-secondary',
  },
  thinking: {
    core: 'rgba(255, 184, 0, 0.65)',
    glow: 'rgba(255, 184, 0, 0.5)',
    ring: 'rgba(255, 184, 0, 0.5)',
    status: 'bg-primary animate-pulse',
  },
  listening: {
    core: 'rgba(171, 235, 255, 0.55)',
    glow: 'rgba(171, 235, 255, 0.4)',
    ring: 'rgba(171, 235, 255, 0.4)',
    status: 'bg-tertiary',
  },
}

const WOLF_BODY_PATHS = [
  'm559.05,895.67c19.63-22.22,37.97-42.97,57.22-64.76-16.36-10.26-21.78-33.45-44.39-32.59-37.06,1.41-79.74-13.81-101.71,35.78,16.54,18.78,35.14,39.92,54.68,62.11-18.04,8.49-34.38,16.3-50.87,23.79-2.3,1.04-6.5,1.74-7.82.48-13.18-12.58-31.45-19.75-36.62-40.6-16.11-64.97-33.4-129.66-50.5-194.38-1.54-5.84-3.9-12.19-7.84-16.53-21.76-23.99-44.14-47.43-67.57-72.42-7.11,6.76-13.81,13.12-21.87,20.78,18.55,21.07,36.86,41.11,54.18,61.97,5.57,6.71,10.15,15.13,12.41,23.52,9.11,33.8,17.13,67.9,26.76,106.79-39.28-23.42-74.49-44.43-109.71-65.42-35.13-20.94-70.27-41.87-107.21-63.87,58.78-50.54,117-100.6,176.07-151.39,2.63,3.98,5.22,6.97,6.81,10.43,15.16,32.98,39.17,53.96,76.14,59.7,5.18.8,12,9.93,12.98,16.02,10.09,62.78,19.12,125.73,28.54,188.62.22,1.5,1.61,2.82,3.07,5.25,19.53-15.66,34.78-29.05,28.36-59.92-11.88-57.17-17.66-115.61-25.64-173.57-1.68-12.23-6.56-20.23-18.59-25.57-28.33-12.56-55.96-26.69-86.35-41.37,28.22-21.82,54.51-42.23,80.9-62.52,29.75-22.87,59.62-45.59,89.36-68.48,6.02-4.63,10.58-8.75,19.06-2.21,55.79,43.04,112.1,85.42,168.19,128.08,1.65,1.26,2.82,3.15,5.25,5.92-30.08,14.63-59.33,28.07-87.65,43.23-6.68,3.58-13.51,12.47-14.71,19.83-10.11,62.23-18.97,124.65-28.12,187.04-3.1,21.11,5.03,33.73,31.76,48.67,4.06-28.98,7.98-57.46,12.05-85.93,4.9-34.3,10.2-68.54,14.71-102.88,1.69-12.89,6.78-21.24,20.63-22.24,1.66-.12,3.25-.98,4.92-1.28,27.31-4.97,47.99-18.05,58.35-45.3,2.91-7.66,8.97-14.11,14.48-22.48,58.99,50.9,116.92,100.88,176.53,152.3-71.98,43.08-142.76,85.44-216.9,129.82,5.56-23.66,10.39-44.26,15.25-64.84,2.87-12.14,4.58-24.73,9.02-36.28,4.56-11.87,10.33-23.87,18.15-33.78,15.45-19.59,32.68-37.77,50.1-57.59-7.26-6.62-14.17-12.92-22.18-20.23-22.12,23.69-45.51,47.63-67.37,72.9-5.88,6.79-7.94,17.4-10.32,26.63-15.78,60.96-29.5,122.52-47.81,182.69-4.81,15.8-23.33,27.49-35.85,40.79-.94,1-4.46.43-6.32-.42-16.84-7.7-33.57-15.62-52.01-24.26Z',
  'm570.46,340.48c89.44-44.25,149.29-122.65,216.91-192.55-7.55,34.44-15.6,67.45-38.12,95.67-19.06,23.88-34,51.01-51.54,76.16-5.62,8.05-6.1,12.82,1.66,19.82,67.95,61.35,135.36,123.28,203.44,184.49,9.38,8.43,15.21,16.69,17.46,29.52,4.8,27.34,11.92,54.26,17.98,81.38.69,3.11.65,6.39.65,6.34-120.99-98.78-243.18-198.55-368.44-300.82Z',
  'm299.36,146.37c4.27,6.51,7.75,13.73,12.93,19.4,26.83,29.42,52.66,59.99,81.63,87.17,26.38,24.75,56.28,45.75,84.76,68.25,10.2,8.06,20.76,15.67,32.65,24.62-37.32,29.04-73.46,57.19-109.65,85.29-24.69,19.17-49.78,37.83-73.97,57.6-7.63,6.24-13.8,7.4-22.97,4.25-20.83-7.16-42.09-13.06-66-20.34,51.6-47.57,101.6-93.68,153.25-141.3-19.94-29.56-38.28-59.56-59.49-87.37-19.94-26.15-28.9-56.22-37.55-86.73-.8-2.81,3.12-6.95,4.85-10.48,0,0-.42-.35-.42-.35Z',
  'm299.36,146.37s.42.34.42.35c-3.29-2.07-6.58-4.14-11.95-7.51-3.88,17.09-7.76,32.71-10.96,48.47-16.26,80.06-32.21,160.17-48.72,240.18-1.4,6.81-4.81,14.24-9.6,19.11-13.06,13.25-27.36,25.28-45.1,41.36,31.43-143.89,61.81-282.95,93.07-426.06,40.85,44.92,79.39,87.3,117.93,129.68-.68.89-1.36,1.77-2.04,2.66-27.68-16.07-55.36-32.15-83.05-48.22Z',
  'm908.33,483.18c-24.57-21.18-51.89-36.92-58.36-74.58-15.06-87.62-35.29-174.36-53.47-261.45-.44-2.11-1.3-4.13-2.69-8.45-31.48,18.52-62.03,36.5-92.57,54.47-.82-.83-1.63-1.65-2.45-2.48,38.14-41.85,76.28-83.7,116.61-127.95,31.46,142.36,62.2,281.4,92.93,420.44Z',
  'm142.9,642.18c7.27-33.72,12.61-68.05,22.82-100.86,3.65-11.74,19.47-19.44,28.99-29.67,10.75-11.56,22.29-12.03,36.55-7.07,17.03,5.93,34.82,9.65,56.52,15.46-48.67,42.93-95.05,83.84-141.42,124.75-1.15-.87-2.3-1.74-3.45-2.61Z',
  'm639,249.33c-34.57,26.11-65.53,49.49-97.12,73.35-32.21-23.86-63.8-47.27-99.01-73.35h196.13Z',
]

const WOLF_JAW_PATH =
  'm541.72,987.53c-15.09-3.86-31.09-6.24-45.4-12.49-5.46-2.38-8.65-13.41-9.7-20.94-.49-3.5,5.83-9.64,10.38-11.83,11.71-5.62,24-10.18,36.39-14.14,4.95-1.58,11.12-1.75,16.11-.34,10.86,3.08,21.67,6.86,31.8,11.8,6.23,3.04,15.94,9,15.72,13.23-.55,11-1.7,24.05-18.12,26.17-12.24,1.58-24.18,5.47-37.17,8.54Z'

/**
 * 3D Wolf Orb — CSS perspective sphere + layered wolf face + pointer tracking.
 * Pure framer-motion/CSS (no Three.js). Respects prefers-reduced-motion via global CSS.
 */
export function WolfOrb({
  size = 'md',
  state = 'idle',
  trackPointer = true,
  showStatus = true,
  showRing = true,
  className = '',
  ariaLabel = 'Hunter Copilot avatar',
}: WolfOrbProps) {
  const px = sizeMap[size]
  const accent = stateAccent[state]
  const wrapRef = useRef<HTMLDivElement>(null)
  const [hovered, setHovered] = useState(false)

  const mx = useMotionValue(0)
  const my = useMotionValue(0)
  const spring = { stiffness: 220, damping: 22, mass: 0.4 }
  const sx = useSpring(mx, spring)
  const sy = useSpring(my, spring)

  const rotateY = useTransform(sx, [-0.5, 0.5], [16, -16])
  const rotateX = useTransform(sy, [-0.5, 0.5], [-14, 14])
  const glossPctX = useTransform(sx, [-0.5, 0.5], [28, 72])
  const glossPctY = useTransform(sy, [-0.5, 0.5], [24, 58])
  const faceX = useTransform(sx, [-0.5, 0.5], [-6, 6])
  const faceY = useTransform(sy, [-0.5, 0.5], [-5, 5])
  const glossBackground = useTransform(
    [glossPctX, glossPctY],
    ([x, y]) =>
      `radial-gradient(circle at ${x}% ${y}%, rgba(255,255,255,0.42) 0%, rgba(255,255,255,0.08) 28%, transparent 55%)`,
  )

  const handleMove = useCallback(
    (e: React.MouseEvent | React.PointerEvent) => {
      if (!trackPointer) return
      const el = wrapRef.current
      if (!el) return
      const r = el.getBoundingClientRect()
      const nx = (e.clientX - r.left) / r.width - 0.5
      const ny = (e.clientY - r.top) / r.height - 0.5
      mx.set(Math.max(-0.5, Math.min(0.5, nx)))
      my.set(Math.max(-0.5, Math.min(0.5, ny)))
    },
    [mx, my, trackPointer],
  )

  const handleLeave = useCallback(() => {
    setHovered(false)
    mx.set(0)
    my.set(0)
  }, [mx, my])

  const isThinking = state === 'thinking'
  const isOnline = state === 'online'

  const layerStyle = useMemo(
    (): MotionStyle => ({
      transformStyle: 'preserve-3d',
      willChange: 'transform',
    }),
    [],
  )

  const statusDot = (
    <span
      className={`absolute z-30 rounded-full ring-2 ring-black/60 ${accent.status}`}
      style={{
        width: Math.max(8, Math.round(px * 0.18)),
        height: Math.max(8, Math.round(px * 0.18)),
        top: '4%',
        right: '2%',
      }}
      aria-hidden
    />
  )

  const ringSize = Math.round(px * 1.38)

  return (
    <div
      ref={wrapRef}
      className={`relative shrink-0 ${className}`}
      style={{ width: px, height: px, perspective: '700px' }}
      onPointerMove={handleMove}
      onPointerEnter={() => setHovered(true)}
      onPointerLeave={handleLeave}
      role="img"
      aria-label={ariaLabel}
    >
      {/* Ambient outer glow */}
      <motion.div
        aria-hidden
        className="pointer-events-none absolute inset-0 rounded-full blur-xl"
        style={{
          background: `radial-gradient(circle at 50% 50%, ${accent.glow}, transparent 70%)`,
          scale: 1.35,
        }}
        animate={{
          opacity: isThinking ? [0.55, 1, 0.55] : hovered ? 0.95 : isOnline ? [0.5, 0.85, 0.5] : 0.55,
        }}
        transition={{
          duration: isThinking ? 0.9 : 2.8,
          repeat: Infinity,
          ease: 'easeInOut',
        }}
      />

      {/* Orbit ring */}
      {showRing && (
        <motion.div
          aria-hidden
          className="pointer-events-none absolute left-1/2 top-1/2 rounded-full border"
          style={{
            width: ringSize,
            height: ringSize,
            marginLeft: -ringSize / 2,
            marginTop: -ringSize / 2,
            borderColor: accent.ring,
            boxShadow: `0 0 16px ${accent.glow}, inset 0 0 12px ${accent.glow}`,
            rotateX: 72,
            rotateZ: 0,
          }}
          animate={{ rotateZ: isThinking ? 360 : 360, opacity: hovered ? 0.95 : 0.65 }}
          transition={{
            rotateZ: { duration: isThinking ? 2.4 : 10, repeat: Infinity, ease: 'linear' },
            opacity: { duration: 0.3 },
          }}
        />
      )}

      {/* Secondary thin orbit */}
      {showRing && size !== 'xs' && (
        <motion.div
          aria-hidden
          className="pointer-events-none absolute left-1/2 top-1/2 rounded-full border border-dashed"
          style={{
            width: Math.round(px * 1.18),
            height: Math.round(px * 1.18),
            marginLeft: -Math.round(px * 1.18) / 2,
            marginTop: -Math.round(px * 1.18) / 2,
            borderColor: 'rgba(255,255,255,0.12)',
            rotateX: 64,
            rotateY: 18,
          }}
          animate={{ rotateZ: -360 }}
          transition={{ duration: 16, repeat: Infinity, ease: 'linear' }}
        />
      )}

      {/* Sphere stack */}
      <motion.div
        className="relative h-full w-full"
        style={{ ...layerStyle, rotateX, rotateY, transformStyle: 'preserve-3d' }}
        animate={{ scale: hovered ? 1.04 : 1 }}
        transition={{ type: 'spring', stiffness: 320, damping: 20 }}
      >
        {/* Sphere shell */}
        <div
          className="absolute inset-0 overflow-hidden rounded-full"
          style={{
            background: [
              `radial-gradient(circle at 32% 28%, rgba(255,255,255,0.28) 0%, transparent 32%)`,
              `radial-gradient(circle at 50% 50%, ${accent.core} 0%, rgba(18,19,20,0.95) 68%, rgba(13,14,15,1) 100%)`,
              'linear-gradient(145deg, #1f2021 0%, #0d0e0f 55%, #121314 100%)',
            ].join(','),
            boxShadow: [
              `inset 0 -10px 22px rgba(0,0,0,0.65)`,
              `inset 0 8px 18px rgba(255,255,255,0.08)`,
              `0 10px 28px rgba(0,0,0,0.55)`,
              `0 0 0 1px rgba(255,255,255,0.1)`,
              `0 0 24px ${accent.glow}`,
            ].join(','),
            transformStyle: 'preserve-3d',
          }}
        >
          {/* Equator / depth band */}
          <div
            className="pointer-events-none absolute inset-0 rounded-full opacity-40"
            style={{
              background:
                'linear-gradient(to bottom, transparent 42%, rgba(0,0,0,0.35) 50%, transparent 58%)',
            }}
          />

          {/* Scanline grid (tech DNA of the site) */}
          <div
            className="pointer-events-none absolute inset-0 rounded-full opacity-[0.12] mix-blend-overlay"
            style={{
              backgroundImage:
                'linear-gradient(to right, rgba(255,255,255,0.4) 1px, transparent 1px), linear-gradient(to bottom, rgba(255,255,255,0.4) 1px, transparent 1px)',
              backgroundSize: '10px 10px',
              maskImage: 'radial-gradient(circle at 50% 50%, black 30%, transparent 75%)',
              WebkitMaskImage: 'radial-gradient(circle at 50% 50%, black 30%, transparent 75%)',
            }}
          />

          {/* Wolf face layer (depth-forward) */}
          <motion.div
            className="absolute inset-0 flex items-center justify-center"
            style={{ transformStyle: 'preserve-3d', x: faceX, y: faceY, translateZ: 18 }}
          >
            <svg
              viewBox="0 0 1080 1080"
              className="h-[72%] w-[72%] drop-shadow-[0_4px_10px_rgba(0,0,0,0.55)]"
              aria-hidden
            >
              <defs>
                <linearGradient id={`wolfGrad-${size}-${state}`} x1="0" y1="0" x2="1" y2="1">
                  <stop offset="0%" stopColor="#ffe08a" />
                  <stop offset="45%" stopColor="#ffb800" />
                  <stop offset="100%" stopColor="#c78a00" />
                </linearGradient>
                <linearGradient id={`wolfGloss-${size}-${state}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#ffffff" stopOpacity="0.35" />
                  <stop offset="55%" stopColor="#ffffff" stopOpacity="0" />
                </linearGradient>
              </defs>
              <g fill={`url(#wolfGrad-${size}-${state})`}>
                {WOLF_BODY_PATHS.map((d) => (
                  <path key={d.slice(0, 24)} d={d} />
                ))}
                <motion.path
                  d={WOLF_JAW_PATH}
                  animate={
                    isThinking
                      ? { y: [0, 42, 0, 18, 0] }
                      : { y: [0, 0, 28, 0, 0] }
                  }
                  transition={{
                    repeat: Infinity,
                    duration: isThinking ? 1.6 : 4.5,
                    ease: 'easeInOut',
                    times: isThinking ? [0, 0.25, 0.45, 0.65, 1] : [0, 0.55, 0.65, 0.78, 1],
                  }}
                />
              </g>
              <g fill={`url(#wolfGloss-${size}-${state})`} opacity={0.5}>
                {WOLF_BODY_PATHS.slice(0, 3).map((d) => (
                  <path key={`g-${d.slice(0, 20)}`} d={d} />
                ))}
              </g>
              {/* Eye glints for orb “face” life */}
              <motion.circle
                cx="420"
                cy="360"
                r="28"
                fill="#fff8d6"
                animate={{ opacity: [0.15, 0.95, 0.15], scale: [0.9, 1.05, 0.9] }}
                transition={{ duration: isThinking ? 1.1 : 3.2, repeat: Infinity, ease: 'easeInOut' }}
                style={{ transformOrigin: '420px 360px' }}
              />
              <motion.circle
                cx="660"
                cy="340"
                r="24"
                fill="#fff8d6"
                animate={{ opacity: [0.9, 0.15, 0.9], scale: [1.05, 0.9, 1.05] }}
                transition={{ duration: isThinking ? 1.1 : 3.2, repeat: Infinity, ease: 'easeInOut' }}
                style={{ transformOrigin: '660px 340px' }}
              />
            </svg>
          </motion.div>

          {/* Specular gloss — tracks pointer */}
          <motion.div
            aria-hidden
            className="pointer-events-none absolute inset-0 rounded-full"
            style={{ background: glossBackground }}
          />

          {/* Bottom rim light */}
          <div
            className="pointer-events-none absolute inset-0 rounded-full"
            style={{
              boxShadow: `inset 0 -2px 8px rgba(255,184,0,0.25), inset 0 1px 0 rgba(255,255,255,0.15)`,
            }}
          />
        </div>

        {/* Thinking arc */}
        {isThinking && (
          <motion.div
            aria-hidden
            className="absolute -inset-1 rounded-full border-2 border-transparent"
            style={{
              borderTopColor: '#ffb800',
              borderRightColor: 'rgba(255,184,0,0.3)',
            }}
            animate={{ rotate: 360 }}
            transition={{ duration: 0.85, repeat: Infinity, ease: 'linear' }}
          />
        )}
      </motion.div>

      {showStatus && statusDot}
    </div>
  )
}
